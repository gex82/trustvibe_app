...
export type EscrowState = 'DRAFT' | 'OPEN_FOR_QUOTES' | 'CONTRACTOR_SELECTED' | 'AGREEMENT_ACCEPTED' | 'FUNDED_HELD' | 'IN_PROGRESS' | 'COMPLETION_REQUESTED' | 'APPROVED_FOR_RELEASE' | 'RELEASED_PAID' | 'ISSUE_RAISED_HOLD' | 'RESOLUTION_PENDING_EXTERNAL' | 'RESOLUTION_SUBMITTED' | 'EXECUTED_RELEASE_FULL' | 'EXECUTED_RELEASE_PARTIAL' | 'EXECUTED_REFUND_PARTIAL' | 'EXECUTED_REFUND_FULL' | 'CLOSED' | 'CANCELLED';
export type ProjectCategory = 'plumbing' | 'electrical' | 'painting' | 'roofing' | 'carpentry' | 'hvac' | 'landscaping' | 'cleaning' | 'general';
...
export type LedgerEventType = 'HOLD_CREATED' | 'RELEASE_FULL' | 'RELEASE_PARTIAL' | 'REFUND_FULL' | 'REFUND_PARTIAL' | 'PLATFORM_FEE_CHARGED' | 'JOINT_RELEASE_PROPOSED' | 'JOINT_RELEASE_SIGNED' | 'EXTERNAL_RESOLUTION_SUBMITTED' | 'OUTCOME_EXECUTED' | 'AUTO_RELEASE_EXECUTED';
...
export interface CaseRecord {
    ...
    projectId: string;
    type: 'ISSUE_HOLD';
    status: 'OPEN' | 'WAITING_JOINT_RELEASE' | 'WAITING_EXTERNAL_RESOLUTION' | 'RESOLUTION_SUBMITTED' | 'ADMIN_ATTENTION_REQUIRED' | 'CLOSED';
