...
async function call<TName extends CallableName>(
  ...
  const fn = httpsCallable<CallableRequest<TName>, CallableResponse<TName>>(
    functions,
    name
  );
...
export async function probeBackend(timeoutMs = 3000): Promise<boolean> {
  const timeout = new Promise<never>((_, reject) => {
    const timer = setTimeout(() => {
      clearTimeout(timer);
      reject(new Error("backend_probe_timeout"));
    }, timeoutMs);
  });
  try {
    await Promise.race([getCurrentConfig(), timeout]);
    return true;
  } catch {
    return false;
  }
...
export function getProject(
  payload: CallableRequest<"getProject">
): Promise<CallableResponse<"getProject">> {
...
export function submitQuote(
  payload: CallableRequest<"submitQuote">
): Promise<CallableResponse<"submitQuote">> {
...
export function fundHold(
  payload: CallableRequest<"fundHold">
): Promise<CallableResponse<"fundHold">> {
...
export function submitReview(
  payload: CallableRequest<"submitReview">
): Promise<CallableResponse<"submitReview">> {
...
export function sendMessage(
  payload: CallableRequest<"sendMessage">
...
export function listInvoices(
  payload: CallableRequest<"listInvoices">
): Promise<CallableResponse<"listInvoices">> {
...
export function adminExecuteOutcome(
  payload: CallableRequest<"adminExecuteOutcome">
): Promise<CallableResponse<"adminExecuteOutcome">> {
  return call("adminExecuteOutcome", payload);
}

export function adminSetConfig(
  payload: CallableRequest<"adminSetConfig">
): Promise<CallableResponse<"adminSetConfig">> {
  return call("adminSetConfig", payload);
}

export async function listCollectionData(
  collectionPath: string,
  max = 50
): Promise<Array<{ id: string } & DocumentData>> {
  const snap = await getDocs(query(collection(db, collectionPath), limit(max)));
...
export function mapApiError(error: unknown): string {
  ...
  if (
    ...
    return "Network unavailable. Check emulator/runtime connectivity.";
  }
